...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList =
    try {
      val current = this.mostRetweeted

      new Cons(current, this.remove(current).descendingByRetweet)
    } catch {
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted = {
    ...
    val leftMax = tryGet(left)
    val rightMax = tryGet(right)

    List(leftMax, rightMax, elem)
      .filter(tweet => tweet != null)
      .maxBy(tweet => tweet.retweets)
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets
    .filter(tweet => google.exists(tag => tweet.text.contains(tag)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets
    .filter(tweet => apple.exists(tag => tweet.text.contains(tag)))
